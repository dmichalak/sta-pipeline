...
def sta_batchruntomo(
    ...
    if batch_dir is None and stack_dir is None:
        print(
            "Specifying either a batch directory (--batch_dir) or a single tilt series directory (--stack_dir) is required."
        )
        raise SystemExit(0)
    elif batch_dir is not None and stack_dir is not None:
        print(
            "Only *one* of either a batch directory (--batch_dir) or a single tilt series directory (--stack_dir) is required."
        )
        raise SystemExit(0)
        ...
        if starting_step == 0:
            ...
            if len(mrc_file) != 1:
                print(
                    "There is either no or more than 1 .mrc file in the tilt series directory."
                )
                raise SystemExit(0)
            else:
    ...
    for directory in dirs_to_process:
        ...
        if fullnofid is False:
            ...
            if nofid == True and starting_step < 9:
                ...
                aligned_stack_without_fiducials = directory / Path(
                    rootname + "_ali_nofid.mrc"
                )
                aligned_stack_with_fiducials = aligned_stack_with_fiducials.rename(
                    directory / Path(rootname + "_ali_fid.mrc")
                )
                aligned_stack_without_fiducials = (
                    aligned_stack_without_fiducials.rename(
                        directory / Path(rootname + "_ali.mrc")
                    )
                )
            ...
            if nofid == True:
                aligned_stack_without_fiducials = (
                    aligned_stack_without_fiducials.rename(
                        directory / Path(rootname + "_ali_nofid.mrc")
                    )
                )
                aligned_stack_with_fiducials = aligned_stack_with_fiducials.rename(
                    directory / Path(rootname + "_ali.mrc")
            ...
            if nofid == True and aligned_stacks_renamed == False:
                print(
                    "WARNING: Aligned stacks were NOT renamed back to their original names."
                )
            ...
            if starting_step is None and ending_step is None:
                for directory in dirs_to_process:
                    ...
                    aligned_stack_with_fiducials = directory / Path(
                        rootname + "_ali.mrc"
                    )
                    aligned_stack_without_fiducials = directory / Path(
                        rootname + "_ali_nofid.mrc"
                    ...
                    with open(f"sta_batchruntomo.out", "a") as out, open(
                        f"sta_batchruntomo.err", "a"
                    ) as err:
                        subprocess.run(batchruntomo_command, stdout=out, stderr=err)
                    aligned_stack_without_fiducials = (
                        aligned_stack_without_fiducials.rename(
                    ...
                    nofid_sl_tomo = nofid_sl_tomo.rename(
                        directory / Path(rootname + "_rec_nofid.mrc")
                    )
                    nofid_bp_tomo = nofid_bp_tomo.rename(
                        directory / Path(rootname + "_rec_BP_rec_nofid.mrc")
                    )
                    job_success(directory, "sta_batchruntomo")
...
@click.option(
    ...
    "-td",
    default=None,
    help="The path to the directory of a single tilt series.",
)
@click.option(
    "--batch_dir",
    "-bd",
...
@click.option(
    ...
    "-s",
    default=None,
    help="""Step to end processing with in each set and axis.  Steps are numbered as... \
                 \n 0: Setup
                 \n 1: Preprocessing
...
@click.option(
    ...
    "-nf",
    is_flag=True,
    help="Indicates that the tilt series with fiducials erased by fidder should be used.",
)
@click.option(
    "--binning",
    "-bin",
...
@click.option(
    "--fullnofid",
    is_flag=True,
    help="Process the tilt series from steps 0 to 20 and include fiducial erasing with fidder.",
)
@click.option(
    "--pixel_spacing", "-ps", help="Pixel spacing for use with --nofid and --fullnofid."
)
@click.option(
    "--probability_threshold",
    "-pt",
    help="Probability threshold for use with --nofid and --fullnofid.",
)
def cli(
    directive_file,
    stack_dir,
